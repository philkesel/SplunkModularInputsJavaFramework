...
public class KafkaModularInput extends ModularInput {
	...
	private static Logger logger = LoggerFactory.getLogger(KafkaModularInput.class);
	...
	protected void doRun(Input input) throws Exception {
		...
		if (input != null) {
			...
			logger.error("Kafka Input is null");
			System.exit(2);
		}
	}
	...
	private void startMessageReceiverThread(String stanzaName, List<Param> params, boolean validationConnectionMode)
		...
		if (!isDisabled(stanzaName)) {
			MessageReceiver mr = new MessageReceiver(stanzaName, topicName, zkhost, zkport, zkchroot,
					zkconnectRawString, groupID, zkSessionTimeout, zkSyncTime, autoCommitInterval,
					additionalConnectionProps, messageHandlerImpl, messageHandlerParams, transport, groupId);
	...
	public class MessageReceiver extends Thread {
		...
		public MessageReceiver(String stanzaName, String topicName, String zkhost, int zkport, String zkchroot,
				String zkconnectRawString, String groupID, int zkSessionTimeout, int zkSyncTime, int autoCommitInterval,
				String additionalConnectionProps, String messageHandlerImpl, String messageHandlerParams,
				Transport transport, String groupId) {
			...
			connectionProperties.put("zookeeper.session.timeout.ms", String.valueOf(zkSessionTimeout));
			connectionProperties.put("zookeeper.sync.time.ms", String.valueOf(zkSyncTime));
			connectionProperties.put("auto.commit.interval.ms", String.valueOf(autoCommitInterval));
		...
		private Map<String, String> getParamMap(String localResourceFactoryParams) {
			...
			try {
				...
				logger.error("Exception processing param map", e);
			}
		...
		private void connect() throws Exception {
			...
			this.consumer = kafka.consumer.Consumer.createJavaConsumerConnector(this.consumerConfig);
			connected = true;

			logger.info("Kafka consumer stanza " + stanzaName + " connected to " + consumerConfig.zkConnect()
					+ " for group" + consumerConfig.groupId() + " receving from " + topicName);
		...
		private void disconnect() {
			...
			connected = false;
			logger.info("Kafka consumer stanza " + stanzaName + " disconnected from " + consumerConfig.zkConnect()
					+ " for group" + consumerConfig.groupId());
		}
		...
		public void run() {
			...
			while (!isDisabled(stanzaName)) {
				while (!connected) {
					try {
						...
						try {
							...
							logger.error("Exception trying to connect " + stanzaName, exception);
						}
					}
				}
				...
				try {
					...
					KafkaStream<byte[], byte[]> stream = consumerMap.get(this.topicName).get(0);
		...
		private void streamMessageEvent(byte[] message) {
			try {
				...
				if (messageCount % 500 == 0) {
					logger.info("Kafka message count[" + this.topicName + "-" + this.groupId + "]: " + messageCount);
				}
			} catch (Exception e) {
				logger.error("Stanza " + stanzaName + " : " + "Error handling message : ", e);
			}
		}
	...
	private void validateConnection(Validation val) throws Exception {
		...
		try {
			...
			throw new Exception("A Zookeeper connection can not be establised with the supplied propertys.Reason : "
					+ t.getMessage());
		}
	...
	protected Scheme getScheme() {
		...
		arg.setDescription(
				"An implementation of the com.splunk.modinput.kafka.AbstractMessageHandler class.You would provide this if you required some custom handling/formatting of the messages you consume.Ensure that the necessary jars are in the $SPLUNK_HOME/etc/apps/kafka_ta/bin/lib directory");
		arg.setRequired_on_create(false);
		endpoint.addArg(arg);
